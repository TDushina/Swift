...
class Snake: SKShapeNode {
    ...
    let moveSpeed = 125.0
    //угол, необходимый для расчёта направления
    ...
    convenience init(atPoint point: CGPoint) {
        ...
        //змейка начинается с головы, создадим её
        let head = SnakeHead(atPoint: point)
        //добавим в массив
        body.append(head)
        //сделаем её дочерним объектом
        addChild(head)
    }
    //перемещаем змейку
    func move () {
    ...
    //метод добавляет ещё один сегмент тела
    func addBodyPart() {
